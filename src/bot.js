...
export default function Bot () {
  let guesses = [], // [ [1,2,3,4], [6,5,4,3], [1,1,1,1], ... ]
    ...
    possible = Array(4).fill(null).map( () => [1, 2, 3, 4, 5, 6] ),
    unordered = null;

  function first() {
    return Array(4).fill(null).map( () => random(6) )
  }
  ...
  function candidate() {
    ...
    else return Array(4).fill(null).map( (n, i) => possible[i][random(possible[i].length) - 1] )
  }
